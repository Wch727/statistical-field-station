...
(function() {
  ...
  function topsis(data, w1, w2) {
    ...
    const result = wnorm.map(d => {
      ...
      const c = sm / (sp + sm);
      return {name: d.name, v1: d.v1, v2: d.v2, sp, sm, c};    result.sort((a,b) => b.c - a.c);
  ...
  function update() {
    ...
    result.forEach(d => {
      ...
      svg.append('text').attr('x',xScale(d.v1)+8).attr('y',yScale(d.v2)-6).text(d.name).style('font-family','var(--mono)').style('font-size','0.7rem').style('fill','var(--forest)').style('font-weight','600');    // Ranking table
...
(function() {
  ...
  pairLabels.forEach((label, idx) => {
    ...
    row.append('span').attr('id',`ahp-v${idx}`).style('font-family','var(--mono)').style('font-size','0.78rem').style('font-weight','600').style('color','var(--terracotta)').style('min-width','36px').style('text-align','right');  const wSvg = d3.select('#ahp-weight-chart').append('svg').attr('width',380).attr('height',160).attr('viewBox','0 0 380 160');

  function update() {
    // Get values
    const pairs = pairLabels.map((_,i) => {
      const idx = +d3.select(`#ahp-p${i}`).property('value');
      return scales[idx];    pairs.forEach((v,i) => d3.select(`#ahp-v${i}`).text(v < 1 ? v.toFixed(2) : v.toFixed(0)));
  ...
  d3.selectAll('[id^="ahp-p"]').on('input', update);
  update();
})();
